...
# Main class for Layout ED2 page
class ED2Layout:
    ...
    def __init__(self, root, load_cart=False):
        ...
        self.root.title("Elphaba Day 2 Seats")
        self.root.configure(bg="#d9e2f1")

        # Placing the main design of the save page
        self.save_title = tk.PhotoImage(file="Designs/Layout/ED2_Layout.png")
        ...
        self.concert_name = "Elphaba - Day 2"
        self.cart = {}
        if load_cart:
            self.load_cart()
    ...
    def load_cart(self):
        self.cart = {}
        with open("tix_cart.dat", "r") as tixCart: # Save the lines in a list
            load = tixCart.readlines()
            self.concert_name = load[0].strip() # Take the concert name
            for line in load[1:]: # Take the seat details after the concert name (2nd line)
                seat, price = line.strip().split(':')
                self.cart[seat] = int(price) # Add them to dictionary
    ...
    def open_cart(self):
        ...
        self.save_title_label.place_forget()
        TixCart(self.root, load_cart=True)
